...
import random
import copy
# from .RBFKANLayer import RBFKANLayer,RBFKANLayerBias
...
class KAN(nn.Module):
    ...
    def __init__(self, width=None, num_basis=5,symbolic_enabled=False, bias_trainable=True,basis_trainable=True,grid_range=None,kernel=...,
        ...
        self.width = width
        # if isinstance(grid_range[0], (int, float)):
        #     grid_range=[grid_range]*self.depth
        ...
        for l in range(self.depth):
            ...
            if grid_range is not None:
                ...
                ranges=None
            sp_batch = KANLayer(in_features=width[l], out_features=width[l + 1],num_basis=num_basis,ranges=ranges,base_fn=base_fn,basis_trainable=basis_trainable,kernel=kernel,**factory_kwargs)
            # if symbolic_enabled:
            ...
            # bias
            bias = nn.Linear(width[l + 1], 1, bias=False, **factory_kwargs).requires_grad_(bias_trainable)
            bias.weight.data *= 0.
    ...
    def log(self):
        for l in range(self.depth):
            print(self.spline_preacts[l].min(),self.spline_preacts[l].max())
    ...
    def update_grid_from_samples(self, x,y):
        '''
        ...
        for l in range(self.depth):
            self.forward(x)
            next_depth=l+1
            proper_min=(x.min()*(self.depth-next_depth)/self.depth+y.min()*(next_depth)/self.depth)/2
            proper_max=(x.max()*(self.depth-next_depth)/self.depth+y.max()*(next_depth)/self.depth)/2
            self.act_fun[l].update_grid_from_samples(self.acts[l],proper_min,proper_max)
        self.plot()
    ...
    def initialize_grid_from_another_model(self, model, x):
        ...
        model(x)
        for l in range(self.depth):
            print(model.acts[l].shape,"acts")
            self.act_fun[l].initialize_grid_from_parent(model.act_fun[l], model.acts[l])

    def forward(self, x,save_acts=True,acts_has_grad=False):
        '''
        KAN forward
        ...
        for l in range(self.depth):
            ...
            x_numerical, preacts, postacts_numerical, postspline = self.act_fun[l](x,return_acts=save_acts,acts_has_grad=acts_has_grad)
    ...
    def set_mode(self, l, i, j, mode, mask_n=None):
        ...
        self.act_fun[l].mask.data[j * self.act_fun[l].in_features + i] = mask_n
    ...
    def train(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0., lamb_coefdiff=0., update_grid=True, grid_update_num=1, loss_fn=None, lr=1., stop_grid_update_step=50, batch=-1,
        ...
        def reg(acts_scale):
            ...
            for i in range(len(self.act_fun)):
                coeff_l1 = torch.sum(torch.mean(torch.abs(self.act_fun[i].weight), dim=1))
                coeff_diff_l1 = torch.sum(torch.mean(torch.abs(torch.diff(self.act_fun[i].weight)), dim=1))
        ...
        #     if 'rbf.center' in name:
        #         rbf_centers.append(param)
        #     else:
        #         other_params.append(param)
        # params=[{'params':rbf_centers,'lr':lr*0.1},{'params':other_params}]
        # print(len(rbf_centers),"rbf centers, ",len(other_params),"other params")
        if opt == "Adam":
            optimizer = torch.optim.Adam(self.parameters(), lr=lr)
        elif opt == "LBFGS":
            optimizer = LBFGS(self.parameters(), lr=lr, history_size=100, line_search_fn="strong_wolfe", tolerance_grad=1e-32, tolerance_change=1e-32, tolerance_ys=1e-32)
        ...
        def closure():
            global train_loss, reg_
            optimizer.zero_grad()
            pred = self.forward(dataset['train_input'][train_id],save_acts=lamb>0,acts_has_grad=lamb>0)
            if sglr_avoid == True:
            ...
            if lamb == 0.:
                reg_ = torch.zeros([1],**self.factory_kwargs)
            else:
        ...
        for _ in pbar:
            ...
            if _ % grid_update_freq == 0 and _ < stop_grid_update_step and update_grid:
                self.update_grid_from_samples(dataset['train_input'][train_id],dataset['train_label'][train_id])
    ...
    def prune(self, threshold=1e-2, mode="auto", active_neurons_id=None):
        ...
        model2 = KAN(copy.deepcopy(self.width),self.num_basis,self.symbolic_enabled,self.bias_trainable,**self.factory_kwargs,seed=self.seed)
