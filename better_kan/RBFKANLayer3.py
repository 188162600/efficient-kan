...
class RBFKANLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.base_fn is not None:
            ...
            postact=(y.sum(dim=2)+self.bias+base).permute(0,2,1)
    ...
    def forward(self, x):
        ...
        if self.base_fn is not None:
            ...
            y=y.unsqueeze(-1) *self.weight.unsqueeze(0)*self.proper_mask().unsqueeze(0).unsqueeze(2)
            postact=((y.sum(dim=2)+base)*self.proper_mask()).permute(0,2,1)
        
            return y.sum(dim=(1,2))+self.bias+(base*self.proper_mask()).sum(dim=1) ,preact,postact,postact
        else:
            preact=x.unsqueeze(1).repeat(1,self.out_features,1)
            y = self.rbf(x)
            y=y.unsqueeze(-1) *self.weight.unsqueeze(0)
    ...
    #     if self.base_fn is not None:
    #         base=self.base_fn(x).unsqueeze(-1)*self.scale_base.unsqueeze(0)*self.proper_mask().unsqueeze(0)
    #         preact=x.unsqueeze(1).repeat(1,self.out_features,1)
    #         y = self.rbf(x)  # [batch, in_features, num_centers]
        
    #         y=y.unsqueeze(-1) *self.weight.unsqueeze(0)*self.proper_mask().unsqueeze(0).unsqueeze(2)
    #         postspline=((y.sum(dim=2))*self.proper_mask())
    #         postact=(postspline+( self.bias+base)*self.proper_mask()).permute(0,2,1)
    ...
    def get_subset(self,in_id,out_id):
        subset=RBFKANLayer(len(in_id),len(out_id),self.num_centers,base_fn=self.base_fn,dtype=self.weight.dtype,device=self.weight.device)
